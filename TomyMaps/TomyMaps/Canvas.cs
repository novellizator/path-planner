...
namespace TomyMaps
{
    public class Canvas
    {
        ...
        public Graphics gr; // sometimes I need direct access
        ...
        public void DrawNormalRectangle(float cx, float cy, float w, float h)
        {

            gr.DrawRectangle(currPen, cx + 0.5f * currPenWidth, cy + 0.5f * currPenWidth, w, h);
        }

        protected void TiltedRectangle(float cx, float cy, float w, float h, float angle)
        {
            double da = Math.PI * angle / 180.0; //Arith.DegreeToRadian(angle);
